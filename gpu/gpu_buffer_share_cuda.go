...
func NewPersistentBufferManager(planet *core.VoxelPlanet) (*PersistentBufferManager, error) {
...
func (mgr *PersistentBufferManager) UpdateFromPlanet(planet *core.VoxelPlanet) {
...
func (mgr *PersistentBufferManager) UpdateToPlanet(planet *core.VoxelPlanet) {
	idx := 0
	for _, shell := range planet.Shells {
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
