...
func (r *VoxelRenderer) HandleMouseClick(xpos, ypos float64, planet *VoxelPlanet) {
	// Only handle clicks in plate mode
	if r.renderMode != 4 || planet.physics == nil || planet.physics.plates == nil {
		return
	}
	...
	if plateID > 0 {
		...
		r.displayPlateInfo(plateID, planet.physics.plates)
	}
}
...
func (r *VoxelRenderer) findPlateAtPosition(pos mgl32.Vec3, planet *VoxelPlanet) int {
	...
	// Look up plate ID
	coord := VoxelCoord{Shell: surfaceShell, Lat: latBand, Lon: lonIdx}
	if plateID, exists := planet.physics.plates.VoxelPlateMap[coord]; exists {
		return plateID
	}
...
func (r *VoxelRenderer) displayPlateInfo(plateID int, plateManager *PlateManager) {
	// Find the plate
