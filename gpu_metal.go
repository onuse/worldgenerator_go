...
kernel void updateTemperature(
    ...
    // Skip air voxels
    if (voxel.materialType == 0) { // MatAir
        return;
    }
...
kernel void updateConvection(
    ...
    // Skip air and water
    if (voxel.materialType == 0 || voxel.materialType == 2) { // MatAir or MatWater
        return;
    }
    ...
    if (voxel.materialType == 6) { // MatGranite
        float avgDensity = 2900.0;
        if (voxels[outerIdx].materialType == 5 || voxels[outerIdx].materialType == 7) { // Basalt or Peridotite
            avgDensity = 2900.0;
        }
...
kernel void advectMaterial(
    ...
    if (shellIdx >= 9) { // Near surface
        ...
        if (cellsToShift > 0 && voxel.materialType > 2) { // Skip air and water
            ...
            if (sourceIdx >= 0 && sourceIdx < (int)gridSize.x) {
                ...
                if (sourceVoxel.materialType > 2) { // Don't copy air/water
                    newVoxel.materialType = sourceVoxel.materialType;
    ...
    if (shellIdx < 9 && voxel.velR > 0.0001) {
        ...
        if (shellIdx < 31) {
            ...
            if (outerIdx < (int)gridSize.x) {
                ...
                if (voxel.materialType == 4 && outerVoxel.materialType != 0) { // MatMagma
...
kernel void updateTemperatureFast(
    ...
    // Skip air voxels
    if (voxel.materialType == 0) return;
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (neighborIdx >= 0 && neighborIdx < (int)gridSize.x) {
            ...
            if (neighbor.materialType != 0) { // Not air
	...
	for shellIdx, shell := range planet.Shells {
		...
		for _, latVoxels := range shell.Voxels {
			for _, voxel := range latVoxels {
				...
				voxelData[voxelIndex].materialType = C.uint8_t(voxel.Type)
	...
	for _, shell := range planet.Shells {
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
				...
				voxel.Type = MaterialType(gpuVoxel.materialType)
