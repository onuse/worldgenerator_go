...
type VoxelAdvection struct {
	...
	tempMaterials [][]VoxelMaterial
}

// NewVoxelAdvection creates an advection simulator
func NewVoxelAdvection(planet *VoxelPlanet, physics *VoxelPhysics) *VoxelAdvection {
		...
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
				...
				if voxel.Type == MatGranite {
					...
					if outerVoxel.Type == MatBasalt || outerVoxel.Type == MatPeridotite {
...
func (va *VoxelAdvection) getViscosity(voxel *VoxelMaterial) float64 {
		...
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
				...
				if voxel.VelR > 0.1 {
					...
					if voxel.Type == MatMagma && upperVoxel.Type != MatAir {
	...
	for shellIdx := startShell; shellIdx < len(va.planet.Shells)-1; shellIdx++ {
		...
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
				...
				if voxel.Type == MatBasalt && voxel.Temperature < 800 {
					// Get material below
				...
				} else if voxel.Type == MatGranite && voxel.Temperature < 800 {
...
func (va *VoxelAdvection) DetectSubductionZones() []SubductionZone {
	...
	for latIdx := range shell.Voxels {
		for lonIdx := range shell.Voxels[latIdx] {
			...
			if voxel.VelR < -0.0001 && voxel.Type == MatBasalt {
				zone := SubductionZone{
					LatIdx: latIdx,
					LonIdx: lonIdx,
...
func (va *VoxelAdvection) advectSurfacePlates(dt float64) {
	...
	newVoxels := make([][]VoxelMaterial, len(shell.Voxels))
	for latIdx := range shell.Voxels {
		newVoxels[latIdx] = make([]VoxelMaterial, len(shell.Voxels[latIdx]))
