...
func NewVoxelMechanics(planet *VoxelPlanet, physics *VoxelPhysics) *VoxelMechanics {
	...
	for shellIdx := range vm.planet.Shells {
		...
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
				...
				Tm := float64(MaterialProperties[voxel.Type].MeltingPoint)
...
func (vm *VoxelMechanics) calculateStrainRate(shellIdx, latIdx, lonIdx int) float64 {
	...
	if lonIdx < len(shell.Voxels[latIdx])-1 {
		...
		lat := getLatitudeForBand(latIdx, shell.LatBands) * math.Pi / 180
...
func (vm *VoxelMechanics) getEffectiveViscosity(voxel *VoxelMaterial) float64 {
...
func (vm *VoxelMechanics) DetectPlateBoundaries() []PlateBoundary {
	...
	for latIdx := range shell.Voxels {
		for lonIdx := range shell.Voxels[latIdx] {
			...
			if !voxel.IsBrittle || voxel.Type == MatAir || voxel.Type == MatWater {
				continue
			}
...
func (vm *VoxelMechanics) applyCollisionEffects(shell *SphericalShell, lat1, lon1, lon2 int, dt float64) {
...
func (vm *VoxelMechanics) applySubductionEffects(shell *SphericalShell, lat1, lon1, lon2 int, dt float64) {
...
func (vm *VoxelMechanics) applyRiftingEffects(shell *SphericalShell, latIdx, lonIdx int, dt float64, riftType string) {
	voxel := &shell.Voxels[latIdx][lonIdx]
