...
func addRandomPlateVelocities(planet *VoxelPlanet, rng *rand.Rand) {
	...
	type velocityCell struct {
		lat       float64
		lon       float64
		velTheta  float32 // North-south velocity
		velPhi    float32 // East-west velocity
	...
	for i := 0; i < numCells; i++ {
		...
		cells[i] = velocityCell{
			lat:       lat,
			lon:       lon,
			velTheta:  speed * float32(math.Sin(angle)),
			velPhi:    speed * float32(math.Cos(angle)),
		...
		for latIdx, latBand := range shell.Voxels {
			...
			for lonIdx := range latBand {
				...
				if nearestCell != nil {
					...
					voxel.VelTheta = nearestCell.velTheta * attenuation
					voxel.VelPhi = nearestCell.velPhi * attenuation
				}
			}
		}
	}
