		...
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
				...
				if avgRadius < earthRadius*0.55 {
					...
					voxel.VelEast = 3e-9 * float32(1+0.5*math.Sin(lat*0.1)) // ~3-4.5 cm/year at equator
...
func (p *VoxelPlanet) UpdateSeaLevel() {
	...
	// Only update if change is significant (more than 20 meters)
	if math.Abs(newSeaLevel-p.SeaLevel) > 20.0 {
		// Gradually move toward target sea level (75% damping)
		p.SeaLevel = p.SeaLevel + 0.25*(newSeaLevel-p.SeaLevel)
	}
}
