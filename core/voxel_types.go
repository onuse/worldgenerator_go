...
type VoxelMaterial struct {
	...
	// Material flow velocity in spherical coordinates (m/s)
	// VelR: radial velocity (positive = outward)
	// VelNorth: latitudinal velocity (positive = northward)
	// VelEast: longitudinal velocity (positive = eastward)
	VelR     float32
	VelNorth float32
	VelEast  float32
...
// Compatibility methods for coordinate system transition
// These provide clearer naming while maintaining backward compatibility

// GetVelocityGeographic returns velocity in geographic frame (north/east/up)
func (v *VoxelMaterial) GetVelocityGeographic() GeographicVelocity {
	return GeographicVelocity{
		VNorth: float64(v.VelNorth), // Current VelNorth represents northward velocity
		VEast:  float64(v.VelEast),  // Current VelEast represents eastward velocity
		VUp:    float64(v.VelR),     // Radial velocity
	}
}

// SetVelocityGeographic sets velocity from geographic frame
func (v *VoxelMaterial) SetVelocityGeographic(vel GeographicVelocity) {
	v.VelNorth = float32(vel.VNorth)
	v.VelEast = float32(vel.VEast)
	v.VelR = float32(vel.VUp)
}

// GetVelocitySpherical returns velocity in current spherical representation
func (v *VoxelMaterial) GetVelocitySpherical() SphericalVelocity {
	return SphericalVelocity{
		VR:     float64(v.VelR),
		VTheta: float64(v.VelNorth),
		VPhi:   float64(v.VelEast),
	}
}

// SetVelocitySpherical sets velocity from spherical representation
func (v *VoxelMaterial) SetVelocitySpherical(vel SphericalVelocity) {
	v.VelR = float32(vel.VR)
	v.VelNorth = float32(vel.VTheta)
	v.VelEast = float32(vel.VPhi)
}
...
// Helper methods
// Note: GetLatitudeForBand is now defined in coordinates.go
