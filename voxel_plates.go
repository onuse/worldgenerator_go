...
type TectonicPlate struct {
	...
	MemberVoxels    []VoxelCoord
	BoundaryVoxels  []VoxelCoord // Edge voxels
	
	// Forces acting on plate
	RidgePushForce  Vector3
	SlabPullForce   Vector3
	BasalDragForce  Vector3
	CollisionForce  Vector3
...
type PlateManager struct {
	...
	VoxelPlateMap   map[VoxelCoord]int  // Which plate each voxel belongs to
	BoundaryMap     map[VoxelCoord]bool // Quick lookup for boundary voxels
	planet          *VoxelPlanet
	nextPlateID     int
}

// NewPlateManager creates a plate manager
func NewPlateManager(planet *VoxelPlanet) *PlateManager {
	return &PlateManager{
		planet:        planet,
		VoxelPlateMap: make(map[VoxelCoord]int),
		BoundaryMap:   make(map[VoxelCoord]bool),
		nextPlateID:   1,
	}
}
...
func (pm *PlateManager) IdentifyPlates() {
	...
	pm.Plates = nil
	pm.VoxelPlateMap = make(map[VoxelCoord]int)
	pm.BoundaryMap = make(map[VoxelCoord]bool)
	...
	visited := make(map[VoxelCoord]bool)
	...
	for latIdx := range shell.Voxels {
		for lonIdx := range shell.Voxels[latIdx] {
			coord := VoxelCoord{Shell: surfaceShell, Lat: latIdx, Lon: lonIdx}
...
func (pm *PlateManager) createPlateFromSeed(seed VoxelCoord, visited map[VoxelCoord]bool) *TectonicPlate {
	plate := &TectonicPlate{
		ID:   pm.nextPlateID,
...
func (pm *PlateManager) shouldBelongToPlate(coord VoxelCoord, refVelTheta, refVelPhi float32) bool {
	...
	if voxel.Type == MatAir || voxel.Type == MatWater || !voxel.IsBrittle {
		return false
	}
	
	// Check velocity similarity (within threshold)
	velThreshold := float32(1e-6) // m/s
	thetaDiff := abs(voxel.VelTheta - refVelTheta)
	phiDiff := abs(voxel.VelPhi - refVelPhi)
	
	return thetaDiff < velThreshold && phiDiff < velThreshold
}

// getNeighborCoords returns the coordinates of neighboring voxels
func (pm *PlateManager) getNeighborCoords(coord VoxelCoord) []VoxelCoord {
	var neighbors []VoxelCoord
	shell := &pm.planet.Shells[coord.Shell]
	...
	neighbors = append(neighbors,
		VoxelCoord{Shell: coord.Shell, Lat: coord.Lat, Lon: eastLon},
		VoxelCoord{Shell: coord.Shell, Lat: coord.Lat, Lon: westLon},
	)
	
	// North/South neighbors (if they exist)
	if coord.Lat > 0 {
		neighbors = append(neighbors, VoxelCoord{Shell: coord.Shell, Lat: coord.Lat - 1, Lon: coord.Lon})
	}
...
func (pm *PlateManager) calculatePlateProperties(plate *TectonicPlate) {
	...
	for _, coord := range plate.MemberVoxels {
		...
		mechanics := pm.planet.physics.mechanics
		if mechanics != nil {
			thickness := mechanics.GetLithosphereThickness(coord.Lat, coord.Lon)
			totalThickness += thickness
		}
		
		// Plate type
		if voxel.Type == MatGranite {
			continentalCount++
		} else if voxel.Type == MatBasalt {
			oceanicCount++
		}
		
		// Position for center calculation
		lat := getLatitudeForBand(coord.Lat, shell.LatBands)
...
func (pm *PlateManager) calculatePlateForces(plate *TectonicPlate) {
	// Reset forces
	plate.RidgePushForce = Vector3{}
	plate.SlabPullForce = Vector3{}
	plate.BasalDragForce = Vector3{}
	plate.CollisionForce = Vector3{}
	...
	for _, coord := range plate.BoundaryVoxels {
		...
		// Get boundary type from mechanics system
		boundaries := pm.planet.physics.mechanics.DetectPlateBoundaries()
		for _, boundary := range boundaries {
			if boundary.LatIdx == coord.Lat && boundary.LonIdx == coord.Lon {
				switch boundary.Type {
				case "divergent":
					// Ridge push force
					force := float64(boundary.Intensity) * 1e12 // Scale factor
					// Simplified - push perpendicular to boundary
					plate.RidgePushForce.X += force
					
				case "convergent":
					if voxel.Type == MatBasalt && plate.Type == "oceanic" {
						// Slab pull for subducting oceanic plates
						plate.SlabPullForce.Y -= float64(boundary.Intensity) * 1e13
					} else {
						// Collision resistance
						plate.CollisionForce.X -= float64(boundary.Intensity) * 1e12
					}
				}
			}
		}
...
func (pm *PlateManager) applyPlateMotion(plate *TectonicPlate) {
	...
	for _, coord := range plate.MemberVoxels {
		...
		// Get voxel position
		lat := getLatitudeForBand(coord.Lat, shell.LatBands) * math.Pi / 180.0
...
func (pm *PlateManager) getAverageVelocity(plate *TectonicPlate) Vector3 {
	if len(plate.MemberVoxels) == 0 {
		return Vector3{}
	}
...
// Vector3 is already defined in types.go
