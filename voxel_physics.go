...
func NewVoxelPhysics(planet *VoxelPlanet) *VoxelPhysics {
	vp := &VoxelPhysics{
		planet:             planet,
		...
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
				...
				if shellIdx > 0 {
					...
					if innerVoxel != nil && innerVoxel.Type != MatAir {
				...
				if shellIdx < len(vp.planet.Shells)-1 {
					...
					if outerVoxel != nil && outerVoxel.Type != MatAir {
...
func (vp *VoxelPhysics) applySurfaceHeatExchange(shell *SphericalShell, temps [][]float32, dt float64) {
	for latIdx := range shell.Voxels {
		for lonIdx := range shell.Voxels[latIdx] {
			voxel := &shell.Voxels[latIdx][lonIdx]
			...
			lat := getLatitudeForBand(latIdx, shell.LatBands) * math.Pi / 180.0
	...
	for shellIdx := range vp.planet.Shells {
		...
		for latIdx := range shell.Voxels {
			for lonIdx := range shell.Voxels[latIdx] {
				...
				// Check for melting
				if voxel.Type != MatMagma && voxel.Temperature > float32(meltingPoint) {
					// Melt to magma
					if voxel.Type == MatBasalt || voxel.Type == MatGranite {
						voxel.Type = MatMagma
				...
				if voxel.Type == MatMagma && voxel.Temperature < float32(meltingPoint-100) {
					// Solidify based on composition
					if voxel.Composition < 0.5 {
						voxel.Type = MatGranite // Felsic magma -> granite
					} else {
						voxel.Type = MatBasalt // Mafic magma -> basalt
					}
					voxel.Density = MaterialProperties[voxel.Type].DefaultDensity
				}
				
				// Water phase changes
				if voxel.Type == MatWater && voxel.Temperature < 273.15 {
					voxel.Type = MatIce
					voxel.Density = MaterialProperties[MatIce].DefaultDensity
				}
				if voxel.Type == MatIce && voxel.Temperature > 273.15 {
					voxel.Type = MatWater
...
func (vp *VoxelPhysics) getRadialNeighbor(sourceShellIdx, targetShellIdx, latIdx, lonIdx int) *VoxelMaterial {
