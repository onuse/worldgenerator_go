...
func (va *VoxelAdvection) advectSurfacePlates(dt float64) {
	...
	for latIdx := range shell.Voxels {
		...
		for lonIdx := range shell.Voxels[latIdx] {
			...
			lonMovement := float32(float64(voxel.VelPhi) * dt * cellsPerMeter)
			latMovement := float32(float64(voxel.VelTheta) * dt * float64(shell.LatBands) / latCircumference)
	...
	// Phase 5: Fill gaps in plates with transient voxels
	...
	// Continental drift data tracked internally
