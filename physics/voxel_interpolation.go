...
func InterpolateVoxelFromInnerShell(innerShell *core.SphericalShell, lat, lon float64) *core.VoxelMaterial {
...
func interpolateLongitude(shell *core.SphericalShell, latIndex int, lon float64) *core.VoxelMaterial {
	if latIndex >= len(shell.Voxels) {
		return &core.VoxelMaterial{Type: core.MatAir}
	}
...
func blendVoxels(v1, v2 *core.VoxelMaterial, frac float32) *core.VoxelMaterial {
	...
	result := &core.VoxelMaterial{
		...
		VelNorth:    v1.VelNorth*(1-frac) + v2.VelNorth*frac,
		VelEast:     v1.VelEast*(1-frac) + v2.VelEast*frac,
